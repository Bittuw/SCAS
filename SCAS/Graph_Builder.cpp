...
{
	auto copy_converters = filter(mysql_converters_data_list);
	auto copy_controllers = filter(mysql_controllers_data_list);
	auto copy_groups = filter(mysql_groups_data_list);
	auto copy_users = filter(mysql_employees_data_list);
	auto copy_groups_in_controllers = filter(mysql_groups_in_controllers_data_list);

	right(copy_groups, copy_users);
	left(copy_converters, copy_controllers);
	binding(copy_groups_in_controllers);
}
